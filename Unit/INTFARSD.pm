package Unit::INTFARSD;
use strict;
use warnings;

sub _pick { return $_[ rand @_ ]; }

# Far Side Screensaver Collection
...
sub info {
  return (
    name     => 'INTFARSD',
    fullname => 'The Far Side Screensaver Collection',
    author   => 'Delrina Software',
    payload  => ['INTFARSD.zip'],
    files    => {
...
sub detail {
  ...
  #module names all start with "FS-"
...
sub edit_antswini {
  ...
  if ( defined $line ) {
    ...
    #} else {
    # Pterodactyl module has special config, it needs a message
    #  $line = "[Pterodactyl]\nPterodactyl=Your Message Here!\n";
  }
